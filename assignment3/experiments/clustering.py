...
class ClusteringExperiment(experiments.BaseExperiment):
    ...
    def __init__(self, details, verbose=False):
        ...
        self._clusters = [2, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20, 25, 30, 35]
        self._old_out = None
    ...
    def __do_perform(self, custom_out=None, main_experiment=None):
        ...
        st = clock()
        j = 0
        save_2_km_labels = None
        save_15_km_labels = None
        save_2_gm_labels = None
        save_15_gm_labels = None
        for k in self._clusters:
            ...
            if k == 2:
                save_2_km_labels = km_labels.copy()
            elif k==10:
                save_15_km_labels = km_labels.copy()



            gmm_labels = gmm.predict(self._details.ds.training_x)
            if k == 2:
                save_2_gm_labels = gmm_labels.copy()
            elif k==10:
                save_15_gm_labels = gmm_labels.copy()
        ...
        ds_2d_2_tsne = pd.DataFrame(np.hstack((self._details.ds.training_x2D, np.atleast_2d(save_2_km_labels).T)),
                             columns=['x', 'y', 'target'])

        ds_2d_15_tsne = pd.DataFrame(np.hstack((self._details.ds.training_x2D, np.atleast_2d(save_15_km_labels).T)),
                             columns=['x', 'y', 'target'])


        ds_2d_2_gmm_tsne = pd.DataFrame(np.hstack((self._details.ds.training_x2D, np.atleast_2d(save_2_gm_labels).T)),
                             columns=['x', 'y', 'target'])

        ds_2d_15_gmm_tsne = pd.DataFrame(np.hstack((self._details.ds.training_x2D, np.atleast_2d(save_15_gm_labels).T)),
                             columns=['x', 'y', 'target'])
        ...
        ds_2d_2_tsne.to_csv(self._out.format('{}_clustering_2_2D.csv'.format(self._details.ds_name)))
        ds_2d_15_tsne.to_csv(self._out.format('{}_clustering_15_2D.csv'.format(self._details.ds_name)))
        ds_2d_2_gmm_tsne.to_csv(self._out.format('{}_clustering_2_gmm_2D.csv'.format(self._details.ds_name)))
        ds_2d_15_gmm_tsne.to_csv(self._out.format('{}_clustering_15_gmm_2D.csv'.format(self._details.ds_name)))
        self.log("Done")
